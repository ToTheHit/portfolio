...
const YT_video = (props) => {
    ...
    let [linkState, setLinkState] = useState(true);
    let [buttonState, setButtonState] = useState(true);
    let [iframe, setIframe] = useState(null);
    ...
    return (
        <div
            className={classNames('YT_video', {['YT_video--enabled']: !iframe}, {[props.className]: props.className})}
            onClick={() => play()}
        >
            {linkState &&
                <div className="YT_video__link" >
                    <picture>
                        <source srcSet={"https://i.ytimg.com/vi_webp/" + props.VideoID +"/maxresdefault.webp"} type="image/webp" />
                        <img
                            className="YT_video__media"
